...
if (!function_exists('LoadRecentlyCommented')) {
    function LoadRecentlyCommented($wacko, $for = '', $limit = 50)
    {
        ...
        if ($ids = $wacko->LoadAll(
            'select min(id) as id from ' . $wacko->config['table_prefix'] . 'pages where ' . ($for ? "super_comment_on like '" . quote($wacko->NpjTranslit($for)) . "/%' " : "comment_on != '' ") . 'group by tag order by id desc'
        )) {
        }
    ...
    foreach ($pages as $page) {
        ...
        if ($access) {
            ...
            print('&nbsp;&nbsp;&nbsp;<span class="dt">'
                  . $time
                  . '</span> &mdash; (<a href="'
                  . $this->href('', $page['tag'], 'show_comments=1')
                  . '#comments">'
                  . $page['tag']
                  . '</a>'
                  . ') . . . . . . . . . . . . . . . . <small>'
                  . $this->GetResourceValue('LatestCommentBy')
                  . ' '
                  . ($this->IsWikiName($page['comment_user']) ? $this->Link('/' . $page['comment_user'], '', $page['comment_user']) : $page['comment_user'])
                  . "</small><br>\n");
        }
    }
} else {
