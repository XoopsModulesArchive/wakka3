...
<div class="page">
    ...
    if ($global = $_REQUEST['global']) {
        ...
        $title = str_replace(
            '%1',
            $this->ComposeLinkToPage($this->GetPageTag()),
            str_replace(
                '%2',
                ($this->GetConfigValue('referrers_purge_time') ? (1 == $this->GetConfigValue('referrers_purge_time') ? $this->getResourceValue('Last24Hours') : str_replace(
                    '%1',
    ...
    if ($referrers) {
        {
            ...
            foreach ($referrers as $referrer) {
                ...
                print('<td valign="top"><a href="' . $referrer['referrer'] . '">' . htmlspecialchars($referrer['referrer'], ENT_QUOTES | ENT_HTML5) . '</a></td>');
    ...
    if ($global) {
        print('<br>[' . str_replace('%1', $this->href('referrers_sites'), str_replace('%2', $this->GetPageTag(), $this->GetResourceValue('ViewReferringSites'))) . ' | ' . str_replace(
            '%1',
            $this->href('referrers'),
            str_replace('%2', $this->GetPageTag(), $this->GetResourceValue('ViewReferrersFor'))
        ) . ']');
    } else {
