...
if (!function_exists('links_tree_view')) {
    function links_tree_view(&$wacko, $node, $level, $indent = 0)
    {
        if ($level > 0) {
            ...
            $pages = $wacko->LoadAll(
                'select to_tag from ' . $wacko->config['table_prefix'] . 'links, ' . $wacko->config['table_prefix'] . "pages where from_tag='" . quote($node) . "' AND " . $wacko->config['table_prefix'] . 'links.to_tag = ' . $wacko->config['table_prefix'] . 'pages.tag order by to_tag asc'
            );

            if (is_array($pages)) {
                $head = preg_split(' :: ', str_replace(']]', '', str_replace('[[', '', $wacko->GetConfigValue('navigation_links'))));
                ...
                foreach ($pages as $page) {
                    ...
                    if ((!in_array($node, $head, true) && $wacko->GetConfigValue('root_page') != $node) || 0 == $indent) {
