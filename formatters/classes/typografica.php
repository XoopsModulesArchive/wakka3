...
class typografica
{
    ...
    public $skipTags = true;

    public $pPrefix = '<p class=typo>';

    public $pPostfix = '</p>';

    public $Asoft = true;

    public $Indent1 = "images/z.gif width=25 height=1 border=0 alt=\'\' align=top>"; // <->
    public $Indent2 = "images/z.gif width=50 height=1 border=0 alt=\'\' align=top>"; // <-->
    public $FixedSize = 80; // максимальная ширина
    public $ignore = "/(<!--notypo-->.*?<!--\/notypo-->)/si"; // regex, который игнорируется.
    public $de_nobr = true;
    ...
    public $glueleft = ["рис\.", "табл\.", "см\.", "им\.", "ул\.", "пер\.", "кв\.", 'офис', "оф\."];
...
'dashglue' => 1,
'wordglue' => 1, // приклеивание предлогов и дефисов
...
'html' => 0, // запрет тагов html
    ...
    public function replaceSpecials($data)
    {
        //print "(($data))";
        ...
        if (($this->settings['quotes']) && (($this->settings['laquo']) || ($this->settings['farlaquo']))) {
            $data = preg_replace(
                "/(\&\#147\;(([A-Za-z0-9'!\.?,\-&;:]|\s|\200|\201)*)&laquo;(.*)&raquo;)&raquo;/i",
                '\\1&#148;',
                $data
            );
        }
    ...
    public function replaceMacros($data, $noParagraph)
    {
        // 1. Абзацы
