...
if ($_REQUEST['confirm']) {
    ...
    elseif ($this->GetConfigValue('allow_registration') || $this->IsAdmin()) {
        ...
        if (!$this->IsWikiName($name)) {
            $error = $this->GetResourceValue('MustBeWikiName');
        } elseif (!$email) {
            $error = $this->GetResourceValue('SpecifyEmail');
        } elseif (!preg_match("/^.+?\@.+$/", $email)) {
            $error = $this->GetResourceValue('NotAEmail');
        } elseif ($confpassword != $password) {
            $error = $this->GetResourceValue('PasswordsDidntMatch');
        } elseif (preg_match('/ /', $password)) {
            $error = $this->GetResourceValue('SpacesArentAllowed');
        } elseif (mb_strlen($password) < 5) {
            $error = $this->GetResourceValue('PasswordTooShort');
        } else {
            $confirm = md5(mt_rand() . $email . mt_rand());

            $more = $this->ComposeOptions(['send_watchmail' => 'Y']);
            ...
            $this->Query(
                ...
                . "', "
                . "bookmarks = '"
                . quote($this->GetDefaultBookmarks($lang))
                . "', "
                . "typografica = '"
                . ((1 == $this->config['default_typografica']) ? 'Y' : 'N')
                . "', "
                . "showdatetime = '"
                . ((1 == $this->config['default_showdatetime']) ? 'Y' : 'N')
                . "', "
                . "more = '"
                . $more
                . "', "
            ...
            $message .= str_replace(
                ...
                str_replace(
                                ...
                                str_replace(
                                    ...
                                    $this->GetResourceValue('Mail.Registered')
                                )
                            )
            ) . '<br> ';
    ...
    if ($this->GetConfigValue('allow_registration') || $this->IsAdmin()) {
        ...
        if ($this->GetConfigValue('multilanguage')) {
            ...
            <tr>
                ...
                <td><input name="name" size="27" value="<?php echo $name ?>"></td>
            </tr>
            <tr>
